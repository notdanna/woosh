...
func renderAppIcon(px: Int) -> Data? {
    ...
    NSGraphicsContext.saveGraphicsState()
    squircle.addClip()
    // Dark minimalist background gradient
    NSGradient(colors: [
        NSColor(calibratedWhite: 0.16, alpha: 1),
        NSColor(calibratedWhite: 0.07, alpha: 1),
    ])?.draw(in: bgRect, angle: -90)

    // Crisp minimalist circle in center (~44% of squircle width)
    let circleDiameter = bgRect.width * 0.44
    let circleRect = NSRect(x: bgRect.midX - circleDiameter / 2,
                            y: bgRect.midY - circleDiameter / 2,
...
func renderMenuBarIcon(scale: Int) -> Data? {
    ...
    // Minimalist circle glyph in the menu bar:
    // Diameter of 8.5pt in logical space (crisp, elegant, not too big).
    let circleDiameter = 8.5 * CGFloat(scale)
    let circleRect = CGRect(x: (CGFloat(width) - circleDiameter) / 2,
                            y: (CGFloat(height) - circleDiameter) / 2,
                            width: circleDiameter,
                            height: circleDiameter)
...
if let rep = bitmapCanvas(markWidth, markHeight), let ctx = NSGraphicsContext(bitmapImageRep: rep) {
    ...
    NSGraphicsContext.saveGraphicsState()
    NSGraphicsContext.current = ctx
    let circleRect = NSRect(x: 16, y: 16, width: 96, height: 96)
