...
# Cleanly removes Woosh and every piece of system state it created:
...
# Also clears the pre-rename "Vorssaint.app" / "Vorssaint Utils.app" if still around.
set -uo pipefail

BUNDLE="com.woosh.utils"
APP="/Applications/Woosh.app"
LEGACY_APP="/Applications/Vorssaint.app"
LEGACY_APP2="/Applications/Vorssaint Utils.app"

echo "▸ Quitting…"
pkill -x Woosh 2>/dev/null || true
...
for candidate in "$APP/Contents/MacOS/Woosh" "$LEGACY_APP/Contents/MacOS/Vorssaint" "$LEGACY_APP2/Contents/MacOS/VorssaintUtils"; do
    if [[ -x "$candidate" ]]; then
...
tccutil reset All "com.vorssaint.utils" >/dev/null 2>&1 || true

echo "▸ Removing app, preferences and saved state…"
rm -rf "$APP" "$LEGACY_APP" "$LEGACY_APP2"
...
RULES="/etc/sudoers.d/woosh-clamshell /etc/sudoers.d/vorssaint-clamshell /etc/sudoers.d/vorssaint-utils-clamshell /etc/sudoers.d/vorss-clamshell"
if ls $RULES >/dev/null 2>&1; then
    echo "▸ Removing closed-lid sudoers rule (asks for your admin password)…"
    osascript -e "do shell script \"rm -f $RULES\" with administrator privileges with prompt \"Woosh uninstaller\"" || true
fi

echo "✓ Woosh fully removed."
