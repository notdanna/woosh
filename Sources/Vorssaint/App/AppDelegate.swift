...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate, NSWindowDelegate {
    ...
    private func showPopover(anchor button: NSStatusBarButton? = nil,
        ...
        popover.show(relativeTo: button.bounds, of: button, preferredEdge: .minY)
        if let window = popover.contentViewController?.view.window {
            makePopoverWindowTransparent(window)
    ...
    private func makePopoverWindowTransparent(_ window: NSWindow) {
        ...
        if let frameView = window.contentView?.superview {
            ...
            stripPopoverVisualEffects(from: frameView, excluding: window.contentView)
        }
    }

    private func stripPopoverVisualEffects(from view: NSView, excluding contentView: NSView?) {
        for subview in view.subviews where subview !== contentView {
            ...
            if let effect = subview as? NSVisualEffectView {
                ...
                if name.contains("Popover") || name.contains("Background") || name.contains("VisualEffect") || name.contains("Frame") {
                    subview.alphaValue = 0
                    subview.isHidden = true
                }
            }
            stripPopoverVisualEffects(from: subview, excluding: contentView)
        }
    }
