...
final class ClipboardHistoryService: ObservableObject {
    ...
    // Height matches the compact size on purpose: activating preview should
    // only widen the window to make room for the sidebar, never make it
    // taller — a vertical jump while browsing history is disorienting.
    static let quickPanelPreviewSize = NSSize(width: 840, height: quickPanelCompactSize.height)
    ...
    private static let persistQueue = DispatchQueue(label: "com.woosh.utils.clipboard-persist",
                                                    qos: .utility)
    ...
    private func captureIfChanged() {
        ...
        GeneralPasteboardAccess.shared.async { [weak self] in
            ...
            DispatchQueue.main.async {
                ...
                switch content {
                case .files(let paths): self.promoteFiles(paths, appBundleID: sourceAppBundleID, appName: sourceAppName)
                case .image(let image): self.promoteImage(image, appBundleID: sourceAppBundleID, appName: sourceAppName)
                case .text(let text): self.promote(text, appBundleID: sourceAppBundleID, appName: sourceAppName)
                }
            }
        }
    }
    ...
    private func promoteImage(_ image: (data: Data, width: Int, height: Int),
        ...
        if let existing = entries.first(where: { $0.kind == .image && $0.imageHash == hash }) {
            ...
            insertPromoted(ClipboardHistoryEntry(id: existing.id,
                                                 ...
                                                 appBundleID: appBundleID ?? existing.appBundleID,
